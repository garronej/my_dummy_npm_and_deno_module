...
## Step 1: Provide port for dependencies the module is using that is not already known by denoify.

This demo project depends on tree modules:

- ``"js-yaml"``: Is is already known by denoify as it is present in : [knownPorts.jsonc](https://github.com/garronej/denoify/blob/master/knownPort.jsonc),  
...
- ``"ts-md5"``: (It is in knownPorts.json but let us assume it is not ) We need to fork the repo and denoify it ourselves,
...
First off, run ``> npm install --save-dev garronej/denoify#v0.2.0`` then add/edit
the ``npm`` scripts.

``package.json``
```json
    "devDependencies": {
        "denoify": "garronej/denoify#v0.2.0",
    }
    "scripts": {
        "tsc": "npx tsc",
        "denoify": "npx denoify",
...
Just after running ``> npm publish`` got to your GitHub repo pages -> release -> create new release ( or draft ne release ) and tag version enter ``v0.1.1`` matching the current version in your ``package.json`` file.
...
```typescript
import { Cat } from "https://deno.land/x/my_dummy_npm_and_deno_module@v0.1.1/mod.ts";
```
or if you haven't published on Deno.land:

```typescript
import { Cat } from "https://raw.github.com/garronej/my_dummy_npm_and_deno_module/v0.1.1/mod.ts";
```
